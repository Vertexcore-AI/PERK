                ...
                <div class="overflow-x-auto">
                    <table class="w-full table-fixed">
                        <thead class="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                <th class="w-2/5 px-3 py-4 text-left text-sm font-medium text-gray-500 dark:text-gray-300 uppercase">Item Description</th>
                                <th class="w-1/5 px-3 py-4 text-left text-sm font-medium text-gray-500 dark:text-gray-300 uppercase">Batch Info</th>
                                <th class="w-1/10 px-3 py-4 text-center text-sm font-medium text-gray-500 dark:text-gray-300 uppercase">Qty</th>
                                <th class="w-1/6 px-3 py-4 text-right text-sm font-medium text-gray-500 dark:text-gray-300 uppercase">Unit Price </th>
                                <th class="w-1/6 px-3 py-4 text-right text-sm font-medium text-gray-500 dark:text-gray-300 uppercase">Total </th>
                                <th class="w-1/6 px-3 py-4 text-center text-sm font-medium text-gray-500 dark:text-gray-300 uppercase">Stock</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @foreach($quotation->quoteItems as $item)
                                <tr>
                                    <td class="px-3 py-4">
                                        <div class="text-sm font-medium text-gray-900 dark:text-white" title="{{ $item->item->description }}">{{ $item->item->description }}</div>
                                        <div class="text-xs text-gray-500 mt-1">Code: {{ $item->item->item_no }}</div>
                                        @if($item->item->manufacturer_name)
                                            <div class="text-xs text-gray-500 mt-1" title="{{ $item->item->manufacturer_name }}">Mfg: {{ $item->item->manufacturer_name }}</div>
                                        @endif
                                    </td>
                                    <td class="px-3 py-4">
                                        <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $item->batch->batch_number }}</div>
                                        <div class="text-xs text-gray-500 mt-1">Available: {{ $item->batch->remaining_qty }}</div>
                                        @if($item->batch->expiry_date)
                                            <div class="text-xs text-gray-500 mt-1">Exp: {{ $item->batch->expiry_date->format('M d, Y') }}</div>
                                        @endif
                                    </td>
