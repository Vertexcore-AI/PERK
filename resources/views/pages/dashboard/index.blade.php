...
<div class="flex items-center gap-4">
    <div class="relative" x-data="{ open: false }">
        ...
        <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-100"
            ...
            <div class="p-2">
                <label
                    class="flex items-center p-2 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-lg cursor-pointer">
...
<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-8">
        ...
        <div class="flex items-start justify-between mb-4">
            <div class="flex gap-4 items-center">
                <div class="w-12 h-12 bg-slate-100 dark:bg-slate-700 rounded-xl flex items-center justify-center">
                    <i data-lucide="box" class="w-6 h-6 text-slate-600 dark:text-slate-400"></i>
                </div>
                <div>
                    <div class="text-2xl font-bold text-slate-900 dark:text-white">
                        {{ $activeItems }}<span class="text-slate-500">/{{ $totalItems }}</span>
                    </div>
                    <h3 class="text-sm font-semibold text-slate-600 dark:text-slate-400">Active Items</h3>
                </div>
            </div>
        ...
        <div class="mt-4">
            <div class="flex items-center justify-between mb-2">
                ...
                <div class="text-right">
                    <span class="text-xs text-slate-900 dark:text-white font-medium">{{ $activeItems }}</span>
                    <span class="text-xs text-slate-400 ml-1">({{ $activePercentage }}%)</span>
                </div>
            </div>
            <div class="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2">
                <div class="bg-primary-600 h-2 rounded-full" style="width: {{ $activePercentage }}%"></div>
            </div>
        </div>
    </div>
    ...
    <div class="card p-6">
        <div class="flex items-start justify-between mb-4">
            <div class="flex gap-4 items-center">
                ...
                <div>
                    <div class="text-2xl font-bold text-slate-900 dark:text-white">
                        LKR {{ number_format($totalProfit, 2) }}
                    </div>
                    <h3 class="text-sm font-semibold text-slate-600 dark:text-slate-400">Total Profit</h3>
                </div>
            </div>
            <button class="btn-icon">
                <i data-lucide="more-vertical" class="w-4 h-4"></i>
        ...
        <div class="mt-4">
            <div class="flex items-center justify-between mb-2">
                <span class="text-xs font-medium text-slate-500">Profit Overview</span>
                <div class="text-right">
                    <span class="text-xs text-slate-900 dark:text-white font-medium">LKR {{ number_format($totalProfit,
                        2) }}</span>
                </div>
            </div>
            <div class="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2">
    ...
    <div class="card p-6">
        <div class="flex items-start justify-between mb-4">
            <div class="flex gap-4 items-center">
                <div class="w-12 h-12 bg-slate-100 dark:bg-slate-700 rounded-xl flex items-center justify-center">
                    <i data-lucide="shopping-cart" class="w-6 h-6 text-slate-600 dark:text-slate-400"></i>
                </div>
                <div>
                    <div class="text-2xl font-bold text-slate-900 dark:text-white">
                        LKR {{ number_format($totalDailySales, 2) }}
                    </div>
        ...
        <div class="mt-4">
            <div class="flex items-center justify-between mb-2">
                ...
                <div class="text-right">
                    <span class="text-xs text-slate-900 dark:text-white font-medium">Avg: LKR {{
                        number_format($avgDailySales, 2) }}</span>
                    <span class="text-xs text-slate-400 ml-1">({{ $dailySalesPercentage }}%)</span>
                </div>
            </div>
            <div class="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2">
                <div class="bg-blue-500 h-2 rounded-full" style="width: {{ $dailySalesPercentage }}%"></div>
            </div>
        </div>
    </div>
    ...
    <div class="card p-6">
        <div class="flex items-start justify-between mb-4">
            <div class="flex gap-4 items-center">
                ...
                <div>
                    <div class="text-2xl font-bold text-slate-900 dark:text-white">
                        {{ number_format($avgDiscount, 2) }}<span class="text-slate-500">%</span>
                    </div>
                    <h3 class="text-sm font-semibold text-slate-600 dark:text-slate-400">Average Discount</h3>
                </div>
            </div>
            <button class="btn-icon">
                <i data-lucide="more-vertical" class="w-4 h-4"></i>
        ...
        <div class="mt-4">
            <div class="flex items-center justify-between mb-2">
                <span class="text-xs font-medium text-slate-500">Discount Overview</span>
                <div class="text-right">
                    <span class="text-xs text-slate-900 dark:text-white font-medium">LKR {{
                        number_format($totalDiscountAmount, 2) }}</span>
                    <span class="text-xs text-slate-400 ml-1">({{ number_format($avgDiscount, 2) }}%)</span>
                </div>
            </div>
            <div class="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2">
                <div class="bg-rose-500 h-2 rounded-full" style="width: {{ $avgDiscount }}%"></div>
            </div>
        </div>
    </div>
...
{{-- <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
    ...
    <div class="xl:col-span-2 card">
        <div class="p-6 border-b border-slate-200 dark:border-slate-700">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    <button
                        class="w-8 h-8 rounded-lg bg-rose-100 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 hover:bg-rose-200 dark:hover:bg-rose-900/50 transition-colors">
                        <i data-lucide="trash-2" class="w-4 h-4 mx-auto"></i>
                    </button>
                    <button
                        class="w-8 h-8 rounded-lg bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 hover:bg-amber-200 dark:hover:bg-amber-900/50 transition-colors">
                        <i data-lucide="refresh-cw" class="w-4 h-4 mx-auto"></i>
                    </button>
                    <button
                        class="w-8 h-8 rounded-lg bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-200 dark:hover:bg-emerald-900/50 transition-colors">
        ...
        <div class="p-0">
            <div
                class="h-80 flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-900">
    ...
    <div class="card overflow-hidden">
            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <a href="#"
                                class="font-medium text-slate-900 dark:text-white hover:text-primary-600 transition-colors">Shopify
                                eCommerce Store</a>
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <a href="#"
                                class="font-medium text-slate-900 dark:text-white hover:text-primary-600 transition-colors">iOS
                                Apps Development</a>
                            <div class="text-xs text-slate-500 dark:text-slate-400">Development</div>
                        </div>
                    </div>
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <a href="#"
                                class="font-medium text-slate-900 dark:text-white hover:text-primary-600 transition-colors">Figma
                                Dashboard Design</a>
        ...
        <a href="#"
            class="block p-4 bg-slate-50 dark:bg-slate-800 text-center text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
            Full Details
        </a>
    </div>
</div> --}}
