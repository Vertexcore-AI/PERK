...
<li class="text-slate-600 dark:text-slate-300">Items</li>
@endsection
...
<div class="flex items-center gap-3">
    ...
    <a href="{{ route('itemcsv.exprot') }}" class="btn-secondary">
            ...
            <div class="flex items-center gap-3">
                <div class="relative">
                    <select id="category-filter"
                        class="px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-primary-500">
            ...
            <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                ...
                <tr class="group" data-category="{{ $item->category_id }}">
                    ...
                    <td>
                        ...
                        <div>
                            @if($batchCount > 0)
                            <span class="font-medium text-slate-900 dark:text-white">{{ $batchCount }} batch{{
                                $batchCount > 1 ? 'es' : '' }}</span>
                    ...
                    <td>
                        <div class="flex items-center gap-2">
                            <span class="text-slate-900 dark:text-slate-100 font-medium">{{ $item->min_stock ?? 0
                                }}</span>
                            <span class="text-slate-500 dark:text-slate-400">{{ $item->unit_of_measure }}</span>
                            @if(($item->min_stock ?? 0) <= $item->reorder_point)
                                <span
                                    class="inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800 dark:bg-amber-900/20 dark:text-amber-300">
                                    Low
                                </span>
                                @endif
                        </div>
                    ...
                    <td>
                        <span class="text-slate-600 dark:text-slate-400">{{ $item->updated_at->format('d/m/Y H:i')
                            }}</span>
                    </td>
                    <td>
                        <div class="flex items-center justify-end gap-1">
                            <a href="{{ route('items.show', $item) }}" class="btn-icon group/btn"
                            ...
                            <a href="{{ route('items.edit', $item) }}" class="btn-icon group/btn"
                                data-tooltip="Edit Item">
                                <i data-lucide="edit"
                                    class="w-4 h-4 text-slate-600 dark:text-slate-400 group-hover/btn:text-primary-600 dark:group-hover/btn:text-primary-400"></i>
                            </a>
                            <button type="button" class="btn-icon group/btn" data-tooltip="Adjust Stock"
                                onclick="adjustStock({{ $item->id }})">
                                <i data-lucide="package-plus"
                                    class="w-4 h-4 text-slate-600 dark:text-slate-400 group-hover/btn:text-blue-600 dark:group-hover/btn:text-blue-400"></i>
                            </button>
                            @if($item->is_serialized)
                            <button type="button" class="btn-icon group/btn" data-tooltip="Serial Numbers"
                                onclick="manageSerials({{ $item->id }})">
                                <i data-lucide="scan-line"
                                    class="w-4 h-4 text-slate-600 dark:text-slate-400 group-hover/btn:text-green-600 dark:group-hover/btn:text-green-400"></i>
                            </button>
                            @endif
                            <form action="{{ route('items.destroy', $item) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn-icon group/btn" data-tooltip="Delete Item"
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mt-6">
    <div class="card p-6">
        ...
        <h4 class="text-2xl font-bold text-slate-900 dark:text-white">{{ $totalItems ?? 0 }}</h4>
        <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">Total Items</p>
    </div>
    ...
    <div class="card p-6">
        ...
        <h4 class="text-2xl font-bold text-slate-900 dark:text-white">{{ $activeItems ?? 0 }}</h4>
        <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">Active Items</p>
    </div>
    ...
    <div class="card p-6">
        ...
        <h4 class="text-2xl font-bold text-slate-900 dark:text-white">{{ $lowStockItems ?? 0 }}</h4>
        <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">Need Reorder</p>
    </div>
    ...
    <div class="card p-6">
        ...
        <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">Inventory Value</p>
    </div>
</div>
@endsection
