...
class SalesService
{
    ...
    private function processSaleItem(Sale $sale, array $itemData): void
    {
        ...
        foreach ($selectedBatches as $batchSelection) {
            ...
            // Get item discount and VAT from the item data
            $discount = $itemData['discount'] ?? 0;
            $vat = $itemData['vat'] ?? 0;

            // Calculate item total with discount and VAT
            $subtotal = $unitPrice * $quantity;
            $discountAmount = $subtotal * ($discount / 100);
            $afterDiscount = $subtotal - $discountAmount;
            $vatAmount = $afterDiscount * ($vat / 100);
            $itemTotal = $afterDiscount + $vatAmount;

            // Create sale item record with discount
